...
```=bash
git clone https://github.com/Chaitu-Tatipamula/Near-NFTeee 
cd Near-NFTeee
yarn && yarn build
```
...
```bash=
near call $NFT_CONTRACT_ID nft_mint '{"token_id": "token-1", "metadata": {"title": "NFTeee", "description": "This token makes you go crazy", "media": "some shitty image"}, "receiver_id": "'$MAIN_ACCOUNT'"}' --accountId $MAIN_ACCOUNT --amount 0.1
```
